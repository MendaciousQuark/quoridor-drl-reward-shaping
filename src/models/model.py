...
class Model:
    ...
    def find_legal_moves(self, state):
        current_position = state['board'].pawn_positions[self.colour]
    ...
    def find_legal_walls(self, state, current_position):
            ...
            for j, cell in enumerate(row):
                ...
                if check_vertical:
                    ...
                    if cell in state['walled_cells'][orientation]:
                        continue
                    partially_legal_walls.append(Move(self.colour, position_formatted, None, 'place', None, None, orientation))
        ...
        for wall in partially_legal_walls:
            if validateMove(wall, state['board'], self):
                legal_walls.append(wall)
        return legal_walls
    
    def find_legal_movement(self, state, current_position):
        moves_to_check = []
        adjacent_opposing_pawn = opposingPawnAdjacent(self.colour, state['board'], current_position)
        ...
        if(adjacent_opposing_pawn[0]):
            ...
            for direction in [opponent_direction, LEFT, RIGHT]:
                #convert the current position to the move format (e.g. [1, 0] -> 'a2' (when i = 1 = 2, j = 0 = a))
                start_formatted = moveNumberToLetter(current_position[1]) + str(current_position[0] + 1)
                #determine the end position of the jump move and format it
                end_position = getDirectionIndex(current_position, direction)
                end_formatted = moveNumberToLetter(end_position[1]) + str(end_position[0] + 1)
                ...
                moves_to_check.append(Move(self.colour, start_formatted, end_formatted, 'jump', None, jump_direction_formatted, None))
        ...
        for direction in [UP, DOWN, LEFT, RIGHT]:
            ...
            #create the move object and add it to the list of moves to check
            moves_to_check.append(Move(self.colour, start_formatted, end_formatted, 'move', direction_formatted, None, None))
        
        legal_moves = []
        for move in moves_to_check:
            if validateMove(move, state['board'], self):
